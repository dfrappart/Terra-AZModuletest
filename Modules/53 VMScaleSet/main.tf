...
#
  ...
  #upgrade_mode                                = var.VMSSUpgradeMode
  zones                                       = var.VMSSZonesList
  zone_balance                                = var.IsZoneBalanced
...
resource azurerm_monitor_autoscale_setting "Terra_VMSSAutoscale" {

  lifecycle {
  ignore_changes = [
    #Ignore change for node count since it is autoscaling
    profile[0].capacity[0].default
    ]
  }
  
  name                                        = "autoscale${var.VMSSName}"
  resource_group_name                         = azurerm_resource_group.Terra_RG.name
  location                                    = azurerm_resource_group.Terra_RG.location
  target_resource_id                          = azurerm_linux_virtual_machine_scale_set.Terra_LinuxVMSS.id
  enabled                                     = true

  profile {

    name                                      = var.VMSSPRofileName
    capacity {
      default                                 = var.VMSSInstanceNumber
      minimum                                 = var.VMSSInstanceMin
      maximum                                 = var.VMSSInstanceMax
    }

    rule {
      metric_trigger {
        metric_name                           = var.ScaleUpRuleMetricName
        metric_resource_id                    = azurerm_linux_virtual_machine_scale_set.Terra_LinuxVMSS.id
        time_grain                            = var.ScaleUpRuleTimeGrain
        statistic                             = var.ScaleUpRuleStatistic
        time_window                           = var.ScaleUpRuleTimeWindow
        time_aggregation                      = var.ScaleUpRuleTimeAggreg
        operator                              = var.ScaleUpRuleOperator
        threshold                             = var.ScaleUpRuleThreshold
      }

      scale_action {
          direction                             = "Increase"
          type                                  = var.ScaleUpActionType
          value                                 = var.ScaleUpActionValue
          cooldown                              = var.ScaleUpActionCooldown
      }
    ...
    rule {
      metric_trigger {
        metric_name                           = var.ScaleDownRuleMetricName
        metric_resource_id                    = azurerm_linux_virtual_machine_scale_set.Terra_LinuxVMSS.id
        time_grain                            = var.ScaleDownRuleTimeGrain
        statistic                             = var.ScaleDownRuleStatistic
        time_window                           = var.ScaleDownRuleTimeWindow
        time_aggregation                      = var.ScaleDownRuleTimeAggreg
        operator                              = var.ScaleDownRuleOperator
        threshold                             = var.ScaleDownRuleThreshold
      }

      scale_action {
        direction                             = "Decrease"
        type                                  = var.ScaleDownActionType
        value                                 = var.ScaleDownActionValue
        cooldown                              = var.ScaleDownActionCooldown
      }
  ...
  notification {
    email {
      custom_emails                           = ["david@teknews.cloud"]
      }
